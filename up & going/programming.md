...
## 练习(Practice)
学习写代码的最重要一步就是实践，有些时候看起来明白了一些道理，但不动手去做，依然是雾里看花。所以，打开你的浏览器控制台，或者是各种编辑器，实实在在的把这些代码都写一遍。如果一切顺利，本书的作者还罗列了一些[练习题](https://github.com/getify/You-Dont-Know-JS/blob/master/up%20%26%20going/ch1.md)，不妨也试着把它们完成！

---

## 回顾(Review)
写代码就像是堆积木一样，但你的脑子里先得有一些 **基本的概念** 来拼装你的大厦：
- 你需要 *操作符*(operators) 来操作相应的值

- 你需要 不同 *类型*(types) 的值来执行不同的行为动作

- 你需要 *变量*(variables) 来保存你的数据

- 你需要 *条件*(conditionals) 来做出决策

- 你需要 *循环*(loop) 来重复执行任务，直到满足停止的条件

- 你需要 *函数*(functions) 来组织你的逻辑块，以便进行复用

- 你还需要 *注释*(code comments) 来提高你代码的可读性

- 而最重要的是 *实践*(practice)，最好的学习写代码的方式就是写代码