...
在 `var a = 2;` 这个例子🌰中，执行的是 **左侧LHS(Left-hand Side)** 模式，与之对应的是 **右侧RHS(Right-hand Side)** 模式。
...
<img src="./assets/what's_scope_building.png" width="200px" />
...
foo(2); // 4
```
![b is not defined](./assets/what's_scope_error.png)
...
如果查询一个 *RHS*，最终找到了，但是做了一些与之类型不符的事，比如对一个字符串进行函数调用、对 `null` 或者 `undefined` 使用点运算符获取属性……那就会抛出 `TypeError` 的错误。

`ReferenceError` 意味着 *作用域* 解析失败的错误；`TypeError` 意味着 *作用域* 解析成功，但是进行了非法操作的错误。

## 回顾(Review)
*作用域* 就是一组 **通过变量名查找变量** 的规则的集合。查找的目的是为了给变量赋值或给函数传参，这属于 *LHS*；查找的目的为了从变量中取值，属于 *RHS*。

*LHS* 和 *RHS* 的查询都会在必要的时候往上层 *作用域* 查找，直到找到变量或者到达 *全局作用域*。

*编译器* 会在运行JS代码前进行编译，拿 `var a = 2;` 这样的语句来说，一般可以分为两步：
1. `var a` 是在这个 *作用域* 中声明变量 `a`，这发生在代码运行前；

2. `a = 2` 会进行 *LHS* 的变量查找，如果找到了就会进行赋值。