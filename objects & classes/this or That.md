...
## What's `this`?
之前我们讨论过，`this` 并非 *author-time* 绑定而是 *runtime* 绑定，并且它的 *上下文(contextual)* 是基于函数的调用时的各种条件而形成的。换而言之，在函数声明时，你不用关心 `this`；在函数调用时，`this` 会如影随形伴你左右。

当函数被调用时，会创建一个 *调用栈(call-stack)*，其中会包含函数调用的各种信息，比如函数如何被调用、传递了什么参数等等……而 `this` 也会存在其中，它会被用于函数的执行期间内。

## 回顾(Review)
`this` 的绑定规则对于有的JS开发者来说是一个持续不断的困惑和折磨，与其不断的猜测、试错、复制粘贴 Stack Overflow 上的答案，还不如花点时间彻底了解 `this` 工作的机制。

要明白 `this` 是什么，首先你要知道 `this` 它不是什么 —— `this` **不是** 指向函数本身，**更别提** 它能指向函数调用时的词法作用域环境了。

`this` 的绑定是在函数调用时才完成的，并且它的具体指向也是由函数调用时的 *调用栈* 所决定的。