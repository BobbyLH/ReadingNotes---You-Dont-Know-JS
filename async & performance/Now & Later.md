...
“并发” 在这个场景下很实用：用户滚动浏览器并触发Ajax请求 和 接受到Ajax请求并渲染视图 两者之间并不冲突，它们可以同时执行(当然这是在 任务/进程层面，而非操作系统层面)：
...
## 任务(Jobs)
ES6 中引入了一个新的机制叫做 “任务队列(Job queue)”，其中 `Promise` 就是使用了这个机制。但是目前为止也没有暴露一个API，让你显示的调用这个能力(当然能用 Promise 来 hack 这个能力)，因此想要清楚的描述这个机制的原理还是挺费劲的：

你不妨将 **任务队列** 理解为在游乐园坐过山车，当一轮结束的时候，如果你还想再来一轮，就得到入口处去排队，这个是事件循环；而若是你直接插队，再一次坐上过山车，这就是任务队列。说白了任务队列中的代码，会在每一轮的事件循环的最后，下一轮事件循环开始之前，去执行。并且，在任务中继续添加的任务(比如`Promise`中再调用`Promise`)，会直接加在这个任务队列的后面。即，这也意味着说，从理论上讲你可以无限添加任务到当前的任务队列中，从而阻塞整个事件循环。
...
👆控制台的输出顺序是 `A C D B` 而不是 `A B C D`，因为任务会在下一次事件循环的tick之前全部执行。 

## 语句的执行顺序(Statement Ordering)
有个可能让你意想不到的事情：我们在书写的代码语句的顺序是从上往下的，但JS引擎很可能并不会按照这个顺序去执行。

在探讨前，我们得明确一个观点：从结果来看，你的JS脚本交给JS引擎去执行，最终的顺序肯定符合你书写的顺序，如果居然不是，那么这个JS引擎肯定违反了规范，你应该立即给它的维护团队提Bug —— 虽然多半都是你的代码有问题。而我们接下去讨论的所有，本质上来看是都没办法直接观察到的。
...
上面的这段代码给JS引擎后，它会发现：“天呐！这段代码太傻了，我还是帮忙优化下吧！”
...
👆所有这一切都发生在JS引擎的执行之前的快速编译阶段，因此即便最终观察到的结果都是一样的，但其本质的实现有很大的不同。

不过JS引擎也不是能优化所有的代码，特别是当这段代码有副作用的时候(比如`getter`、`setter`、ES6的 `Proxy`)：
...
如果没有 `console.log(a + b)` 这段代码，JS引擎可能会轻松的完成优化。不过说不定它已经这么做了：
...
这段讨论的主要目的是为了让我们明白：你写的源码和JS引擎最终编译后执行的代码，很可能是完全不一样的。

## 回顾(Review)
JS的程序在绝大多数情况下，都会被拆分成多个块去执行，每个块都共享全局的状态和作用域。

事件循环的每一次迭代都被称为 tick，无论是用户的交互、IO、定时器，都会被加入到某一次的 tick 中。

在任何时候，事件队列中只能有一个 tick 被处理，某个事件的执行过程都可以直接或者间接的往事件队列中添加新的事件。

JS中的并发性是指两个或以上的事件几乎同时被加入了事件队列中，尽管在同一时刻只能有一个事件被执行。

想尽办法确保执行的顺序，以避免诸如 "race conditions" 之内的情况发生。当然，面对一个耗时甚久的同步代码时，也能采用诸如 “时间切片” 的办法将它拆分成更小的块，以避免堵塞。
