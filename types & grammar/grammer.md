...
- [这个例子🌰](https://codepen.io/bobby_li/pen/WqNwLO?editors=1011)说明了标签配合`continue` 和 `break`是如何工作的
...
- 下面代码，第一段会报错，因为变量 `c` 被单独分隔成一行，ASI会自动在b后面添加分号，而 `c` 不存在，因此会出现 `ReferenceError`;
第二段不会，因为逗号将 `c` 连接起来，是一个声明语句
  ```javascript
  var a = 42, b
  c;
  ...
  c;
  ```
  ![缺少逗号](./assets/grammer_err_asi.png)
...
- 错误处理
  - `try catch finally` 语句
    ...
    - [举例子🌰](https://codepen.io/bobby_li/pen/WqpOKz?editors=1111)
...
- [举例子🌰](https://codepen.io/bobby_li/pen/xoqegZ?editors=1111)